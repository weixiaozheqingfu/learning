...
public class OauthAction extends BaseAction {
     ...
     * 这样下次认证时，如果已经登陆过了且没有过期，可以不用再次输入用户名和密码。但仍需要用户点击【确认】或【取消】按钮，逻辑上保证是否授权是由用户决定的。
     * 2.该授权登录和服务器正常登陆域名分开，cookie写入的域名和服务器端的session对象生成各自用一套，授权后，只是登陆了该授权系统，会往客户端对应的授权域名写cookie。
     * 这样下次认证时，如果已经在授权服务器登陆过了且没有过期，可以不用再次输入用户名和密码。但仍需要用户点击【确认】或【取消】按钮，逻辑上保证是否授权是由用户决定的。
