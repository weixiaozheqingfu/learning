...
import com.glitter.spring.boot.context.log.ServiceInputLogInfoContext;
import com.glitter.spring.boot.context.log.ServiceOutputLogInfoContext;
...
public class ServiceLogAspect {
    ...
    public void before(JoinPoint joinPoint) throws Throwable {
        try {
            ...
            logger.info("service log before end,输入参数:{}", JSONObject.toJSONString(serviceInputLogInfo));
        } catch (Exception e) {
    ...
    public void afterReturning(JoinPoint joinPoint, Object ret) throws Throwable {
        try {
            ...
            logger.info("service log afterReturning end,输出参数:{}", JSONObject.toJSONString(serviceOutputLogInfo));
        } catch (Exception e) {
            logger.error(TemplateUtil.getExceptionLogMsg(this.getClass().getName(), Thread.currentThread().getStackTrace()[1].getMethodName(), e, RequestLogInfoContext.get()));
            throw (e instanceof BusinessException) ? (BusinessException) e : new BusinessException(CoreConstants.REQUEST_PROGRAM_ERROR_CODE, "系统异常");
        }
    ...
    public void afterThrowing(JoinPoint joinPoint, Exception ex){
        try {
            ...
            if(ex instanceof BusinessException){
                logger.error("service log afterThrowing end,目标方法业务异常:{}", JSONObject.toJSONString(serviceOutputLogInfo));
            } else {
                logger.error("service log afterThrowing end,目标方法运行异常:{}", JSONObject.toJSONString(serviceOutputLogInfo));
            }
        } catch (Exception e) {
            logger.error(TemplateUtil.getExceptionLogMsg(this.getClass().getName(), Thread.currentThread().getStackTrace()[1].getMethodName(), e, RequestLogInfoContext.get()));
        }
