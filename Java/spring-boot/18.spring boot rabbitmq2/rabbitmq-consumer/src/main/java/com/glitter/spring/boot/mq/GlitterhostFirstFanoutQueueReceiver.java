...
//    }
    ...
    // 2.建议如果消费失败,就将信息导入死信队列,重新放入队列,因为既然第一次不能被正常消费,一般是因为业务问题,再次排队,一般情况下还会是失败。
    // 3.如果设置了手动确认,而客户端代码中又没有做任何确认,那么这条消息就会一直在当前客户端阻塞着,直到当前客户端宕机/重启/停止服务,该消息才会重新入队排到队头.
    //   但此时其他客户端代码和当前客户端代码一致,就会再被其他客户端继续阻塞,造成mq队列消息堆积,很危险,所以一定要记得手动确认。
